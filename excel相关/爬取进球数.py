...
while current_date <= end_date:
    ...
    url = f'https://trade.500.com/jczq/?playid=270&g=2&date={formatted_date}'
    ...
    Balls = [th.text.strip() for th in directory.find_all('td', 'td-betbtn')]
    Boneo = [th.text.strip() for th in directory.find_all(attrs={'data-value':'0'})]
    Bonea = [th.text.strip() for th in directory.find_all(attrs={'data-value':'1'})]
    Boneb = [th.text.strip() for th in directory.find_all(attrs={'data-value':'2'})]
    Bonec = [th.text.strip() for th in directory.find_all(attrs={'data-value':'3'})]
    Boned = [th.text.strip() for th in directory.find_all(attrs={'data-value':'4'})]
    Bonee = [th.text.strip() for th in directory.find_all(attrs={'data-value':'5'})]
    Bonef = [th.text.strip() for th in directory.find_all(attrs={'data-value':'6'})]
    Boneg = [th.text.strip() for th in directory.find_all(attrs={'data-value':'7'})]
