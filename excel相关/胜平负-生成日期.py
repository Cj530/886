...
while current_date <= end_date:
    ...
    def extract_datas(divs, data_value):
        result = []
        for div in divs:
            p_tag = div.find('p', {'data-value': data_value})
            if p_tag:
    ...
    divs = directory.find_all('div', class_='betbtn-row itm-rangB2')

    #主让胜
    victorys = extract_datas(divs, '3') 

    flats = extract_datas(divs, '1') 

    negatives = extract_datas(divs, '0') 


    # 主队让球
    theHomeTeamHandicaps = [th.text.strip() for th in directory.find_all('p', 'green')]
...
output_file = f'{start_date_input}_to_{end_date_input}.json'
final_df.to_json(output_file, orient='records', force_ascii=False)
